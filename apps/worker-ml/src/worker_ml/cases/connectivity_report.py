...
async def load_case_resolution_connectivity_report(
    ...
    *,
    ...
    async with session_factory() as session:
        ...
        resolution_jobs_failed = int(
            await session.scalar(
                select(func.count()).select_from(Job).where(_resolution_job_filter(JOB_STATUS_FAILED))
            )
            or 0
        )
        resolution_jobs_unfinished = int(
        ...
        linked_after_succeeded_resolution = int(
            await session.scalar(_linked_after_succeeded_resolution_query())
            or 0
        )
        unconnected_after_succeeded_resolution = int(
            await session.scalar(_unconnected_after_succeeded_resolution_query())
            or 0
        )
        examples = tuple(
...
def _case_candidate_filter() -> object:
    return ArticleCard.is_case_candidate.is_(True)


def _resolution_job_filter(status: str = JOB_STATUS_SUCCEEDED) -> object:
    return (
        (Job.job_type == RESOLVE_ARTICLE_CASES_JOB)
        & (Job.article_id.is_not(None))
        & (Job.status == status)
